...
| 容器界面内左键 | 两次点击栏位可整堆移动；熔炉界面用统一栏位 `0..38`，箱子界面用统一栏位 `0..62`（`0..35` 玩家物品、`36..62` 箱子 27 格）；普通背包可点击石砖、熔炉、铁块、石镐和铁镐五条固定配方 |
...
当前图形背包的固定合成面板仍只有石砖、熔炉、铁块、石镐、铁镐五个可点击入口，尚未加入箱子的合成按钮；箱子物品目前只能通过直接发送协议 `CraftRecipe`（`Recipe` 字段填 `core.RecipeChest`）获得，图形客户端本身没有暴露该入口。本批不实现大箱子合并、箱子命名、排序、快捷搬运、拆分堆、漏斗、比较器、潜行放置或任何自动化。
...
- 每区块最多 16 个箱子，每个箱子固定 27 格且接受任何已注册物品；每名玩家同时只能查看一个容器，打开箱子会结束熔炉查看关系，反之亦然；尚无大箱子合并、命名、排序、快捷搬运、拆分堆、漏斗、比较器、潜行放置或任何自动化；图形背包的固定合成面板仍只有五条可点击入口，未加入箱子按钮，箱子物品目前只能通过直接发送协议 `CraftRecipe` 请求获得；
...
- 线上协议为 v12；v11 及所有其他不匹配版本都会在握手阶段、进入 Play 前被稳定拒绝，不提供版本协商或降级解码。容器相关消息在 v12 中改用容器中性命名（`OpenContainer`/`MoveContainerStack`/`CloseContainer`/`ContainerClosed`）但保留原有 packet ID，并新增固定长度的 `ChestState`；既有 packet ID 不重排，已废止的 Play client packet ID `1` 继续保持未分配；
- 世界 metadata 保持 v2，记录绝对世界时间。既有 v1 世界可直接打开，世界时间从 `0` 开始，并在下一次正常自动保存或关服时写为 v2；只认识 v1 的旧程序遇到 v2 metadata 会按未来版本稳定拒绝且不覆盖原文件；
- 玩家存档保持 schema v4 不变；区块存档从 schema v5 升级为 schema v6，在熔炉之后追加固定 16 个箱子槽（27 格 × 每格 5 字节，每区块固定负载增加 2304 字节）。v6 程序可无损读取并迁移 v1–v5 的历史区块，v5 迁移为空箱子集合、v1–v4 沿既有链继续升级；只认识 v5 及更早版本的旧程序遇到 v6 区块会按未来 schema 稳定拒绝且不覆盖原文件。读取旧 schema v3 玩家档或 schema v4 区块档时，旧工具会迁移为满耐久；新格式写回后，v11 程序无法读取升级后的区块存档；
- **备份与回退**：升级前必须正常关服，等待进程退出并备份完整世界目录，再启动 v12 程序完成首次写入。区块一旦被写为 schema v6，只认识 v5 及更早的旧程序无法再打开该世界目录。回退时必须先停止服务，再用升级前的完整备份整体恢复世界目录，不能让旧程序直接读取已经写入 v6 的目录。异常进程退出时玩家文件与区块文件各自原子，但两者之间没有跨文件事务：崩溃可能只保留其中一侧的最新提交，正常关服会同时刷写两者；
- benchmark 报告保持 scenario v12：`remote_gpu_complete` 采用批量分摊计时，相对回归门禁只作用于分辨率足够的指标，阶段之间有固定冷却。M4K 新增的箱子由玩家放置，固定种子的 benchmark 世界中不出现任何箱子，地形、运动、阶段与指标定义都不受影响，只有区块固定负载随新增箱子槽增加；只允许显式 `--allow-scenario-upgrade 10:12` 迁移（v11 从未成为基线），v6–v11 历史报告仍可读取。
