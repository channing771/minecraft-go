...
- 尚无投掷/丢弃、掉落物物理与移动；生存、怪物等完整玩法尚未完成。

## 兼容性与升级

- 线上协议为 v9，在既有固定长度 `PlayerState` 末尾追加绝对世界时间；声明 v8 或其他不匹配版本的客户端会在握手阶段、进入 Play 前被稳定拒绝，不提供版本协商或降级解码。已废止的 Play client packet ID `1` 保持未分配，不会复用；
...
- 升级前必须正常关服，等待进程退出并备份包含玩家与区块存档的整个世界目录。回退旧程序时必须恢复首次写入 metadata v2 之前取得的备份，不能直接打开已经保存过 v2 metadata 的世界；
- benchmark 报告为 scenario v13：still/flying 帧与交互客户端一样包含程序化天空 draw（太阳、月亮与星空）；`remote_gpu_complete` 继续采用批量分摊计时，相对回归门禁只作用于分辨率足够的指标，阶段之间有固定冷却。只允许显式 `--allow-scenario-upgrade 12:13` 迁移（v11 从未成为基线，v12 是 M4G 已接受基线），v6–v12 历史报告仍可读取。回退到 M4G 时需同时回退天空 draw、v13 比较规则与 v13 基线，恢复 v12 基线，世界与玩家数据无需迁移。
