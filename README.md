...
- 线上协议为 v8；声明 v7 或其他不匹配版本的客户端会在握手阶段、进入 Play 前被稳定拒绝，不提供版本协商或降级解码。已废止的 Play client packet ID `1` 保持未分配，不会复用；
- 玩家存档保持 schema v3，区块存档保持 schema v4，二进制布局都未改变；玩家 schema v3 现可在原有物品字段中保存石镐和铁镐，采掘进度不持久化。不认识工具 ID 的旧程序会拒绝玩家文件且不得覆盖；
- 升级前必须正常关服，等待进程退出并备份包含玩家与区块存档的整个世界目录。若回退到旧程序，必须恢复首次保存工具之前取得的备份，不能直接打开已经保存过工具的世界。
