...
项目仍处于早期开发阶段。目前已经具备程序化地形、GPU 地形渲染、玩家移动与碰撞、客户端预测、方块挖掘与放置、内置权威服务端、世界持久化、有界二进制协议、TCP 直连、无图形专用服务端以及稳定玩家状态存档；已完成的 M3 多人里程碑支持最多八名玩家通过局域网专用服务端同步移动、角色和 Unicode 昵称；M4A 增加了服务端权威的固定 9 格快捷栏，挖掘获得资源、选择栏位与放置消耗构成最小闭环。
...
- 快捷栏固定 9 格、单格最多 64 个同类物品；快捷栏没有空间时挖掘会被服务端拒绝，方块保持不变；
- 尚无掉落物实体、背包界面与合成，挖掘成功后的物品直接进入快捷栏；
- 生存、合成、光照、怪物等完整玩法尚未完成。

## 兼容性与升级

- 线上协议为 v3：放置命令引用快捷栏栏位，并新增选中栏位请求与完整快捷栏状态消息；声明 v2 及其他版本的客户端会在进入 Play 前被稳定拒绝，不提供版本协商；
- 玩家存档 schema 升级为 v2，保存快捷栏与选中栏位；旧的 v1 存档会被确定性迁移为空快捷栏并在下一次正常保存时写为 v2，世界与区块存档格式不变；
- 升级前请备份世界目录中的玩家存档。回退到旧程序时，尚未写为 v2 的玩家无需处理，已写为 v2 的玩家必须从备份恢复；旧程序会把 v2 当作未来版本稳定拒绝，不会覆盖。
