...
- 列顶高度表与直射天空光都由权威方块派生，不写入区块存档，也不进入网络 payload；程序化天空只消费现有权威世界时间，不增加协议或存档字段；
...
- benchmark 报告为 scenario v13：still/flying 帧与交互客户端一样包含程序化天空 draw（太阳、月亮与星空）；`remote_gpu_complete` 继续采用批量分摊计时，相对回归门禁只作用于分辨率足够的指标，阶段之间有固定冷却。M4K/M4L 没有改变固定种子 benchmark 的地形、运动、阶段或指标定义；诊断性实测（非新基线，见 [`docs/notes/perf-baseline.md`](docs/notes/perf-baseline.md)）确认八人探针玩家在测量窗口内不发生摔落伤害。当前 M5 基线已由一次性 Memory/TCP 正式链提升为 scenario v13，M2 基线保持 scenario v6；比较器只允许显式 `--allow-scenario-upgrade 12:13` 迁移，v6–v12 历史报告仍可读取。回退到 M4L 时需同时回退天空 draw、v13 比较规则与 M5 v13 基线，恢复 M5 v12 基线，世界与玩家数据无需天空相关迁移。
