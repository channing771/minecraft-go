...
var (
	...
	// 未知字段、尾随数据、空计划、未交付步骤类型或非法坐标。上层把它映射
	// 为 InvalidPlan 类任务失败原因，且不重试、不降级、不改写。
	ErrPlannerInvalidPlan = errors.New("companion: planner 返回非法计划")
)

// plannerSystemPrompt 是每次规划请求携带的固定系统提示：声明用户消息是不可
// 信的观察数据、限定输出为单一受限 JSON object、步骤只允许 go_to。M5B 不存在
// 伙伴设定文本，任何此类内容都不进入规划输入。
const plannerSystemPrompt = "你是体素游戏 Mornlea 里伙伴的行动规划器。" +
	...
	"格式为 {\"summary\":\"中文一句话摘要\",\"steps\":[{\"kind\":\"go_to\",\"x\":整数,\"y\":整数,\"z\":整数}]}。" +
	"steps 必须非空且按执行顺序排列；kind 只允许 go_to；x、y、z 必须是十进制整数，" +
	"y 只能在 [-64, 319] 范围内；不要发明其他字段或步骤类型。"
...
// 解码为受限 go_to 计划。不重试、不缓存、不并发（在途请求上限由上层编排负责）；
...
// 未知字段、尾随数据、空计划、未交付 kind、非法坐标）包装 ErrPlannerInvalidPlan。
...
// decodePlanResponse 把（已限长的）响应正文严格解码为计划：先宽容解出唯一
// choice 的 content，再对 content 用 DisallowUnknownFields + 尾随数据检查的
// json.Decoder 解出计划中间形并做全部 schema 校验。任何失败都包装
// ErrPlannerInvalidPlan，错误文本不含 content 原文。
func decodePlanResponse(body []byte) (Plan, error) {
	...
	for index, step := range wire.Steps {
		// kind 必须逐字等于 go_to：大小写敏感，follow/mine/place 等一律拒绝。
		if step.Kind != "go_to" {
			return Plan{}, fmt.Errorf("companion: planner 计划 steps[%d] kind 未交付: %w", index, ErrPlannerInvalidPlan)
		}
		// 指针为 nil 覆盖字段缺席与 JSON null 两种情形，二者都不是有限整数。
		if step.X == nil || step.Y == nil || step.Z == nil {
			return Plan{}, fmt.Errorf("companion: planner 计划 steps[%d] 坐标不是整数: %w", index, ErrPlannerInvalidPlan)
		}
		plan.Steps = append(plan.Steps, PlanStep{Kind: PlanStepGoTo, X: *step.X, Y: *step.Y, Z: *step.Z})
	}
