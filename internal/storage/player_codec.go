...
func encodePlayer(save PlayerSave) ([]byte, error) {
	...
	// schema v3 不能表示磨损值；过渡期只允许满耐久工具进入编码器，
	// 避免解码 shim 把磨损值静默补满。Task 6 升级到 schema v4 时与该 shim 一并删除。
	if !playerInventoryRepresentableV3(save.Inventory) {
		return nil, fmt.Errorf("%w: player schema v3 cannot encode worn tool durability", ErrCorrupt)
	}
...
func decodePlayerStack(decoder *byteDecoder) (core.ItemStack, error) {
	...
	// schema v3 的物品负载只有 Item/Count，没有耐久字节；编码器已拒绝磨损工具，
	// 因此解码时只会恢复满耐久工具。Task 6 升级到 schema v4 读取真实字段时
	// 必须把这个 shim 与编码可表示性门禁一并删除。
...
func playerInventoryRepresentableV3(inventory core.Inventory) bool {
