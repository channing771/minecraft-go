...
采掘进度位于快捷栏正上方中央，使用固定宽度的深色轨道和比例填充。可采目标保留绿色语义并带亮色末端标记；不可采目标保留琥珀色语义并带固定警示缺口，确保两者不只靠颜色区分。进度、所需 tick 与可采状态仍完全来自权威采掘 overlay。
...
零尺寸 framebuffer 继续产生空布局。背包或容器打开时，本 change 不改其格子、面板或命中几何，只要求生存 HUD 保持在 framebuffer 内且不覆盖当前可交互格子；完整容器视觉在下一 change 重排。
...
- 新增 `hud-survival-feedback`，同一帧固定展示低血、耗氧、磨损工具、可采进度及不可采视觉标记中的可观察组合。
