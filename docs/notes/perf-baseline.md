...
2026-08-09 在冻结 HEAD `86ae0cc160732e597ad1ace7497feed091d324e4` 及已记录的 Task 8 工作树上完成 M4N 无窗口 record-only 链。报告身份为 `Apple M2 / 16GiB`、`macOS 26.5.1`（build `25F80`）、`go1.26.0 darwin/arm64`、`2560x1440`。

- 当前 Memory 基线：`docs/notes/perf-baseline.json`，SHA-256 `0df4b40c491a13ab7133b452598f5d669969ca5ee91d91c722fa56f568e7c9d7`
- Memory 报告：`/private/tmp/mcgo-m4n-v15/memory-v15.json`，SHA-256 同上
- TCP 报告：`/private/tmp/mcgo-m4n-v15/tcp-v15.json`，SHA-256 `ced23986ba41bfa957bfc95012336e271bdfc402902b7bfb69bb4b07585175f3`
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Memory / still | 11515 | 191.9 | 5.087ms | 5.764ms | 6.150ms | 69.196ms | 1407.3MiB |
| Memory / flying | 46962 | 391.5 | 1.711ms | 7.969ms | 18.555ms | 135.119ms | 1426.5MiB |
| TCP / still | 11483 | 191.4 | 5.105ms | 5.812ms | 6.224ms | 60.282ms | 1412.1MiB |
| TCP / flying | 45500 | 379.3 | 1.739ms | 8.918ms | 17.290ms | 106.492ms | 1474.5MiB |

Memory/TCP load 为 `39.118516417/39.100708209s`，snapshot 为 `22.443870166/22.566399875s`，cooldown 均为 `30s`。tick p99 为 `0.494541/0.560916ms`；persistence p99 为 `24.162083/24.522459ms`；`remote_gpu_complete` 均为 `128` 样本、每样本 `256` 次绘制，p99 为 `0.224047/0.206545ms`；多人探针 peak RSS 为 `1426.5/1515.6MiB`。完整字段保存在两份 JSON，Task 8 报告逐项列出。
