...
版本与存档兼容性：服务端和客户端必须是同一版本。线上协议为 v4，声明 v3 或更早版本的客户端会在进入 Play 前被稳定拒绝，不提供版本协商或降级解码。玩家存档 schema 为 v2；区块存档 schema 升级为 v2，新增固定 32 个掉落物槽，旧的 v1 区块会被确定性迁移为空掉落物集合并在下一次正常保存时按 v2 原子写回。首次用本版本启动前，请按上述步骤正常关服并备份整个世界目录：回退到旧程序时，尚未写为 v2 的记录无需处理，已写为 v2 的必须从备份恢复，旧程序会把 v2 当作未来版本稳定拒绝而不会覆盖。
