...
func TestPerfcheckOnlyAuthorizesScenarioV16ToV17(t *testing.T) {
	v16 := completeV16ComparableReport("memory")
	v17 := completeV17ComparableReport("memory")
	if records, err := compareReportsWithScenarioUpgrade(v16, v17, 0.20, "16:17"); err != nil || len(records) != 0 {
		t.Fatalf("16:17 迁移 records=%v error=%v", records, err)
	}
	for _, test := range []struct {
		from, to int
		allow    string
	}{
		{15, 16, "15:16"}, // 上一代唯一迁移，本变更起退役
		{14, 15, "14:15"},
		{15, 17, "15:17"},
		{17, 16, "17:16"},
		{16, 16, "16:16"},
...
func TestPerfcheckV17PerformanceRegressionIsRecordOnly(t *testing.T) {
	baseline := completeV17ComparableReport("memory")
	current := completeV17ComparableReport("memory")
...
func TestPerfcheckMultiplayerScenarioUpgradeAndProvenanceRules(t *testing.T) {
	v16 := completeV16ComparableReport("memory")
	v17 := completeV17ComparableReport("memory")
	if _, err := compareReports(v16, v17, 0.20); err == nil || !strings.Contains(err.Error(), "scenario_version") {
		t.Fatalf("default cross-scenario comparison error=%v", err)
	}
	if failures, err := compareReportsWithScenarioUpgrade(v16, v17, 0.20, "16:17"); err != nil || len(failures) != 0 {
		t.Fatalf("explicit 16:17 migration failures=%v error=%v", failures, err)
	}
	for _, test := range []struct {
		name  string
		...
		t.Run("empty "+test.name, func(t *testing.T) {
			baseline, current := v16, v17
			test.clear(&baseline)
			test.clear(&current)
			if _, err := compareReportsWithScenarioUpgrade(baseline, current, 0.20, "16:17"); err == nil ||
	...
	for _, test := range []struct {
		...
		{name: "reverse", allow: "16:17", baseline: v17, current: v16},
		{name: "retired 15:16", allow: "15:16", baseline: completeV15ComparableReport("memory"), current: v16},
		...
		{name: "skip 15:17", allow: "15:17", baseline: completeV15ComparableReport("memory"), current: v17},
	...
	v17.Hardware = "different"
	if _, err := compareReportsWithScenarioUpgrade(v16, v17, 0.20, "16:17"); err == nil ||
...
func TestPerfcheckScenarioUpgradeMatrix(t *testing.T) {
	tests := []struct {
		...
		{16, 17, "", true},       // 无授权跨场景拒绝
		{16, 17, "16:17", false}, // 唯一允许的显式迁移
		{17, 16, "16:17", true},  // 反向参数拒绝
		{15, 16, "15:16", true},  // 上一代唯一迁移退役
		{14, 15, "14:15", true},  // 旧迁移退役
		{6, 16, "6:16", true},    // 独立基线不得伪装成迁移
		...
		{16, 16, "16:17", true},  // 同场景不得忽略未使用授权
		{17, 17, "16:17", true},  // 同场景不得忽略未使用授权
		{17, 17, "", false},      // v17 同场景
		{16, 16, "", false},      // v16 同场景
		{15, 15, "", false},      // v15 同场景
		{14, 14, "", false},      // v14 同场景
...
func TestPerfcheckScenarioUpgradeSkipsRelativeRegressions(t *testing.T) {
	baseline := completeV16ComparableReport("memory")
	...
	failures, err := compareReportsWithScenarioUpgrade(baseline, current, 0.20, "16:17")
...
func TestPerfcheckScenarioUpgradeRejectsIncompleteV17Report(t *testing.T) {
	for _, test := range []struct {
		name, want string
		mutate     func(*client.PerfReport)
	}{
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := compareReportsWithScenarioUpgrade(baseline, current, 0.20, "16:17"); err == nil ||
				...
				t.Fatalf("incomplete v17 error=%v want=%q", err, test.want)
			}
		})
	}
}
