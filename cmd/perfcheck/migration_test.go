...
func TestPerfcheckOnlyAuthorizesScenarioV15ToV16(t *testing.T) {
	...
	if records, err := compareReportsWithScenarioUpgrade(v15, v16, 0.20, "15:16"); err != nil || len(records) != 0 {
		t.Fatalf("15:16 迁移 records=%v error=%v", records, err)
	}
...
func TestPerfcheckV16PerformanceRegressionIsRecordOnly(t *testing.T) {
	...
	current := completeV16ComparableReport("memory")
...
func TestPerfcheckMultiplayerScenarioUpgradeAndProvenanceRules(t *testing.T) {
	v15 := completeV15ComparableReport("memory")
	v16 := completeV16ComparableReport("memory")
	if _, err := compareReports(v15, v16, 0.20); err == nil || !strings.Contains(err.Error(), "scenario_version") {
		t.Fatalf("default cross-scenario comparison error=%v", err)
	}
	if failures, err := compareReportsWithScenarioUpgrade(v15, v16, 0.20, "15:16"); err != nil || len(failures) != 0 {
		t.Fatalf("explicit 15:16 migration failures=%v error=%v", failures, err)
	}
	for _, test := range []struct {
		name  string
	...
	for _, test := range []struct {
		...
		{name: "reverse", allow: "15:16", baseline: v16, current: v15},
		{name: "retired 14:15", allow: "14:15", baseline: completeV14ComparableReport("memory"), current: v15},
		{name: "retired 13:14", allow: "13:14", baseline: completeV13ComparableReport("memory"), current: completeV14ComparableReport("memory")},
		{name: "skip 14:16", allow: "14:16", baseline: completeV14ComparableReport("memory"), current: v16},
	...
	v16.Hardware = "different"
	if _, err := compareReportsWithScenarioUpgrade(v15, v16, 0.20, "15:16"); err == nil ||
...
func TestPerfcheckScenarioUpgradeMatrix(t *testing.T) {
	tests := []struct {
		...
		{15, 16, "", true},       // 无授权跨场景拒绝
		{15, 16, "15:16", false}, // 唯一允许的显式迁移
		{16, 15, "15:16", true},  // 反向参数拒绝
		...
		{15, 15, "15:16", true},  // 同场景不得忽略未使用授权
		{16, 16, "15:16", true},  // 同场景不得忽略未使用授权
		{16, 16, "", false},      // v16 同场景
		{15, 15, "", false},      // v15 同场景
		{14, 14, "", false},      // v14 同场景
		{13, 13, "", false},      // v13 同场景
...
func TestPerfcheckScenarioUpgradeSkipsRelativeRegressions(t *testing.T) {
	...
	failures, err := compareReportsWithScenarioUpgrade(baseline, current, 0.20, "15:16")
...
func TestPerfcheckScenarioUpgradeRejectsIncompleteV16Report(t *testing.T) {
	for _, test := range []struct {
		name, want string
		mutate     func(*client.PerfReport)
	}{
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := compareReportsWithScenarioUpgrade(baseline, current, 0.20, "15:16"); err == nil ||
				...
				t.Fatalf("incomplete v16 error=%v want=%q", err, test.want)
			}
		})
	}
}
