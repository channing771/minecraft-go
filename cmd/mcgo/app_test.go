...
func TestApplicationConnectionRemoteLoginSuccessReturnsOwnedApplicationAfterGraphics(t *testing.T) {
	...
	if app.blockOutlineRenderer == nil || len(app.remoteNameTags) != 0 || cap(app.remoteNameTags) != 8 {
		t.Fatalf("block outline/name-tag scratch=%p/%d/%d，想要已构造且 0/8",
			app.blockOutlineRenderer, len(app.remoteNameTags), cap(app.remoteNameTags))
	}
...
func TestMultiplayerBenchmarkReservesTargetNameTagSlotWithoutAddingTarget(t *testing.T) {
	...
	if len(scenario.Tags) != 7 || cap(scenario.Tags) != 8 {
		t.Fatalf("benchmark tags len/cap=%d/%d，想要 7/8", len(scenario.Tags), cap(scenario.Tags))
	}
