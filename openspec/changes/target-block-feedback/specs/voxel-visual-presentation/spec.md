...
轮廓 MUST 以十二根细长立方体覆盖单位方块包围盒的十二条边，并以固定向外偏移避免与地形共面。它 MUST 位于世界实体之后、HUD 之前的 alpha pass，使用现有深度附件进行深度测试、不得写入深度，并启用 alpha 混合；被目标本体或其他地形遮挡的边 MUST 不得穿透显示。所有当前注册方块 MUST 有非空中文显示名，未知 ID 查询 MUST 失败。
...
- **GIVEN** 七名远端玩家、一个有效目标和已完成初始化的渲染器
- **WHEN** 连续准备与上传稳定目标帧
- **THEN** name-tag 实例数 MUST 不超过八个，轮廓实例数 MUST 不超过十二个
- **AND** 稳定态准备与上传 MUST 不产生堆分配
