...
- **GIVEN** 调用方显式请求更新视觉基线
- **WHEN** M5 无窗口抓帧生成包含 `skylight-tunnel` 与末尾 `block-light-room` 的新图像集
- **THEN** 调用方 MUST 人工复核全部场景图像后才能接受新 golden，且任一场景未收敛、方块光语义不成立或比对超过既有阈值 MUST 失败
