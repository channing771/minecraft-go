...
- [ ] 1.3 在 `cmd/mcgo/benchmark.go` 与 `cmd/mcgo/multiplayer_benchmark.go` 最小实现 scenario v8、单一 2048 样本常量和探针内部测试时钟；标签准备、命令编码与资源释放保持计时区间外，不修改 still/flying workload、分辨率或交互路径。
...
- [ ] 2.2 记录精确 HEAD、`Apple M5 / 24GiB`、OS、GVM Go 1.26.0、电源/低功耗状态、可见系统负载、M2 JSON/Markdown 哈希和两个不存在的全新 `/tmp/mcgo-m5-v8-<HEAD12>-{memory,tcp}.json` 路径；向用户报告“Memory/TCP 各一次、任一步失败停止且不得重跑”的边界，并取得针对该 HEAD 与路径的明确确认。
