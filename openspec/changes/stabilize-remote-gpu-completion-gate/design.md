...
新增单一常量 `remoteGPUCompletionSamples = 2048`，同时用于 recorder 容量、采样循环、报告完整性和测试。2048 样本的 p99 约由第 21 慢样本决定，低于 1% 的孤立系统调度尖峰不会由三次事件支配；若慢样本持续达到 1%，p99 仍会如实退化。预计额外耗时约 2–4 秒，相对固定场景总时长可接受。
