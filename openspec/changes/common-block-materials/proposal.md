...
线上协议由 v13 升为 v14，玩家 schema 由 v5 升为 v6，区块 schema 由 v6 升为 v7；字节布局不变，旧数据执行 identity migration，future schema 与未知编号明确拒绝。服务端权威边界和现有并发模型不变。渲染热路径继续使用固定 atlas、单一 terrain pass 与 8 字节实例格式，性能数值只记录，真实 overflow、数据丢失、报告结构和 I/O 错误仍失败。回退代码后，新 schema 会作为 future schema 被旧程序明确拒绝。
