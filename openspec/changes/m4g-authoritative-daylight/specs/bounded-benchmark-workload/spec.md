...
加入权威世界时间、metadata 保存、区块高度派生状态、直射天空光网格属性和昼夜渲染后的 benchmark 报告 MUST 标记为 scenario v11；既有 scenario v6/v7/v8/v9/v10 报告与基线 MUST 保持可读取，比较器不得把不同 scenario 当作同一工作负载静默相对比较。v10 与 v11 之间 MUST 只通过显式授权迁移，且迁移只执行完整性与绝对门禁。
...
#### Scenario: v10 与 v11 不静默混比
- **WHEN** baseline 为 scenario v10、current 为 scenario v11 且没有显式迁移授权
- **THEN** 比较器 MUST 拒绝相对比较并说明场景版本不一致

#### Scenario: 显式授权 v10 到 v11 迁移
- **WHEN** 调用方显式授权 `10:11` 迁移且两份报告的硬件身份一致
...
#### Scenario: 退役的 v9 到 v10 迁移参数被拒绝
- **WHEN** 调用方使用已经退役的 `9:10` 迁移参数比较 scenario v9 与 v10
...
#### Scenario: 历史报告保持可校验
- **WHEN** 调用方单独读取一份完整 scenario v6、v7、v8、v9 或 v10 报告
- **THEN** 比较器 MUST 按该历史场景原有的完整性规则校验，不得要求历史报告满足 v11 新字段
...
scenario v8 及后续场景（包括 v11）MUST 继续使用现有 still、flying、RSS、服务端 tick、GPU 和 Memory/TCP 比较阈值；昼夜与直射天空光工作量不得提高 `20%` 相对回归阈值或放宽绝对门禁。

#### Scenario: v11 飞行尾延迟超限
- **WHEN** scenario v11 的 flying p99 大于或等于 `12ms`
- **THEN** 性能门禁 MUST 失败

#### Scenario: v11 GPU 稳定分位数退化超限
- **WHEN** 同硬件、同 scenario v11 的 `remote_gpu_complete` 任一受检分位数退化超过 `20%`
- **THEN** 性能门禁 MUST 失败，不得因功能增加而提高阈值

#### Scenario: v11 权威 tick 绝对门禁保持不变
- **WHEN** scenario v11 的服务端 tick p99 达到既有绝对上限
- **THEN** 性能门禁 MUST 失败，不得因增加世界时间或 metadata 保存而放宽上限
