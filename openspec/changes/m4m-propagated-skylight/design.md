...
- 以 scenario v14 与唯一 `13:14` 迁移保持性能数据可解释，将所有性能数值作为记录，并用完整 M5 Memory/TCP 报告更新基线。
...
失败边界只保留数据正确性：报告损坏、缺字段或样本不完整，scenario 迁移未授权或方向不兼容，硬件身份不兼容，Memory/TCP 的 transport、scenario 或 commit 身份不一致，真实 overflow 或数据丢失，以及 I/O 错误。队列高水位是观测数值，不等同于 overflow。绑定临时路径、静稳快照、一次性正式授权、失败即停和禁止重跑不再属于执行前提；复现环境、命令和输出哈希仍可随报告记录。

M5 v14 基线由完整有效的 Memory 报告精确字节提升；同一硬件、scenario 和 commit 的 TCP 报告只需完整记录，Memory/TCP 性能比较无论好坏都不阻止提升。M2 内容和路径保持不变，显式基线路径选择、硬件一致性和唯一 `13:14` 结构迁移继续防止无意义混比。
...
4. 生成完整有效的 M5 Memory/TCP v14 记录，输出不阻塞的性能比较，并以 Memory 报告精确字节替换 M5；不要求绑定路径、静稳预检或一次性授权。
