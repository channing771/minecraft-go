...
- benchmark 升级为 scenario v14，并只允许显式 `13:14` workload 迁移；完整的 M5 Memory 报告可立即精确更新 M5 基线，TCP 报告可独立记录，只有调用方显式请求时才执行跨 transport 比较。
...
- `hardware-performance-baselines`：要求以完整 M5 v14 Memory 报告立即精确替换 M5 当前基线，TCP 独立记录且只在调用方显式请求时参与不改变基线状态的跨 transport 比较，M2 保持不变。
