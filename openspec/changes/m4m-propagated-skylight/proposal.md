...
- benchmark 升级为 scenario v14，并只允许显式 `13:14` workload 迁移；完整的 M5 Memory/TCP 报告用于记录比较，并以 Memory 报告精确更新 M5 基线。
...
- `hardware-performance-baselines`：要求以完整 M5 v14 Memory 报告精确替换 M5 当前基线，TCP 只提供完整记录与不阻塞提升的跨 transport 比较，M2 保持不变。
