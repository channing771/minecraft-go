...
- 首个冻结候选 `f7d8f261e910863e189666f6e2181e606996f42f` 的唯一一次正式 Memory producer 因 flying p99 `12.175ms` 与进程峰值 RSS `2452.2MiB` 违反既有绝对门禁而停止，未生成报告、未运行 TCP、未覆盖基线；后续先修复实现，不放宽 p99、RSS 或其他门禁。
...
- 性能修复会使用不可提升的非正式诊断运行定位 Go 堆、原生图形资源与 fullscreen shader 成本；旧候选及其失败步骤保持冻结，新正式链必须绑定新的实现提交、全新路径和新的明确授权。
