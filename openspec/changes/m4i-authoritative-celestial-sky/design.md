...
动机见 `proposal.md`。当前 M4L 客户端在 `render.DayNightAt` 中从最后确认的 `WorldTimeTicks` 得到 `Sun`、`Daylight` 和一项整屏 `ClearColor`；`cmd/mcgo.renderFrame` 把同一 `Daylight` 传给 terrain、avatar 和 item-drop renderer。`render.Renderer.Render` 已拥有第一个带 color/depth clear 的 terrain pass，先执行 GPU cull，再绘制地形。`Camera` 只带 `ViewProj`、位置、昼夜亮度和 clear color；`cameraBytes` 每帧临时分配编码切片。

M4I 最终基于已归档的 M4L 实现：协议 v13、玩家 schema v5、区块 schema v6、世界 metadata v2 和 scenario v12 已由代码、主规格与两份未改写的基线确认。本 change 不改协议或存档，只把包含天空 draw 的 workload 推进到 scenario v13。早期实现与 M4H–M4L 并行开发；正式性能链前必须合并当前 M4L，并重新冻结候选。
...
- 不改变 avatar、item-drop、name-tag 或 hotbar pipeline；尤其不修改 M4L 已归档的主动丢弃、工具耐久、权威箱子、权威生命值、死亡结算、协议、模拟、镜像、持久化和 renderer 语义。
...
程序化天空改变 still/flying 每一帧的 GPU workload，因此不能与 M4L 保留的 scenario v12 静默相对比较。producer、完整性校验和 `perfcheck` 只增加精确 `12:13` 迁移；该迁移继续只做报告完整性、硬件一致性和现有绝对门禁，不做跨 workload 相对比较。建立 v13 后，`12:13` 以外的旧迁移参数全部退役；历史 v6-v12 报告仍可单独校验和同场景比较。
...
星空短路候选 `4410dc8b5ec76acad7d5a28980ca88b83434d35f` 完成了 pre-M4H 的全仓门禁和不可提升诊断，但远端 `main` 随后归档到 M4H `15f2cf84d6b7e7186b17a0efb6d1ab5c019201e0`。M4I 在 `6badbf4f35ad3e2f5d8047761857c5d39b6cc3ca` 合入 M4H，并在合入 M4J `34c9ba56ad213076c876ab5fffde791ed45ba6fb` 后于候选 `a859fcbc0abd63b4d39fc4321c2facf6fd57a63a` 完成完整门禁；远端 `main` 随后又依次归档 M4K 与 M4L `17807e580ca5675bc216d1e6b3d45adc1782de67`。这些旧候选及诊断只证明 shader 与 encoded MemoryStore 的局部结论，不得进入正式迁移、TCP 比较或基线。

合并时保留 M4L 的 protocol v13、玩家 schema v5、区块 schema v6、权威箱子、生命值、摔落伤害、死亡结算、主动丢弃、工具耐久、迁移、golden 与 HUD 呈现，同时保留 M4I 的权威天空、MemoryStore 编码保留、scenario v13 和唯一 `12:13` 迁移。`README.md`、`cmd/mcgo/app.go`、`app_test.go` 与性能记录冲突按两侧能力并存解决；不得删除任一侧测试、二进制 golden 或把 M4L 的 protocol/schema 值退回。M4K/M4L 明确保持默认 benchmark workload 与两份基线字节不变，因此合并后仍使用 scenario v13；若冲突解决必须改变 draw、分辨率、阶段、样本、运动或指标定义，则先升级 v14。

合并完成后重新运行天空、主动丢弃、工具耐久、权威箱子、生命值、死亡结算、Memory/TCP、协议、存储、迁移/golden/fuzz、全仓 race、vet、archcheck、相关 benchmark 和 OpenSpec strict；通过后提交新的精确 HEAD。只有该 HEAD 完成五分钟冷却、两次静稳预检并绑定两个全新路径后，才能请求新的正式授权。
...
- `internal/storage/memory.go`、`memory_test.go` 与聚焦内部测试：MemoryStore 使用当前 chunk v6 编码 payload 保留区块，并保持批次原子性与无别名读取。
...
1. 核对 M4L 已完成规格同步和归档，并确认协议 v13、玩家 schema v5、区块 schema v6、metadata v2、scenario v12 与两份未改写的基线；若任一不同，先修订本 change 并重新严格校验。
...
6. 合并当前 M4L 后重新运行完整门禁并冻结新候选；所有旧候选与诊断只作不可提升证据，不复用其授权或路径。
7. 新 Memory 通过相应场景迁移的完整性/绝对门禁后只执行一次 TCP，同场景跨 transport 通过后才提升 M5 Memory 精确字节。M2 文件保持不变。
8. 回退时同时回退天空 renderer、场景比较规则和新 M5 基线，恢复 M4L 的 v12 基线；保留 M4L protocol v13、权威箱子、生命值、主动丢弃与工具耐久能力，全部世界/玩家数据无需迁移。
