...
- `internal/storage/memory.go`、`memory_test.go` 与聚焦内部测试：MemoryStore 使用现有 chunk v4 编码 payload 保留区块，并保持批次原子性与无别名读取。
