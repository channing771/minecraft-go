...
- [ ] 7.1 在 `cmd/mcgod` 完成真实磁盘纵向迁移测试，并保留 `internal/storage` 的 `ChunkKeys`/`Backup` 专项测试；覆盖完整备份、稳定顺序、七种值迁移、非自然负载保留、保存/进度/完成标记失败续跑、最终 partial batch 与完成后幂等和 future schema 拒绝，运行 `go test ./internal/storage ./cmd/mcgod -race -count=1`。
