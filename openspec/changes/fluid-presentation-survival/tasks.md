...
- [x] 10.1 实测**流动前沿**两个场景的单 tick 最坏耗时：玩家挖穿大坝、注水世界里的瀑布。`authoritative-fluid` 遗留的残余风险是 `Queue.Advance` 成本正比于队列规模而非预算（约 176–256 ns/项/tick，约 20 万项时单独吃满 50 ms）。**数值只记录，但权威 tick 被无界工作阻塞是门禁。**
