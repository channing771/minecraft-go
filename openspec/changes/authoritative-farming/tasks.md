...
- [x] 9.2 **先用已知坏样本自检检测器**再信任其「0 条」结论——一个恒真的扫描器与不扫描等价,而它看起来像扫过了。验证:`go test ./internal/archcheck -count=1` **结论**:扫描器内核 `scanCommentBacktickIdentifiers` 吃 map[文件名][]byte、不碰磁盘,自检因此与全仓门禁**走同一条代码路径**。三个真实仓库样本逐个验过并已移除:(a) 在 `internal/sim/engine.go` 真实注释里写一个不存在的名字 → EXIT=1,报告精确指到 `engine.go:103:27`;(b) 改成两个存在的名字(`advanceWorldTime`、`core.ChunkKey`) → EXIT=0;(c) 复刻 F2 坏样本,把 `internal/worldgen/oracle_test.go` 的 `oracleSeaLevel` 改名为 oracleSeaLevelY 而 `fluid_test.go:26` 的注释不改 → EXIT=1,指到 `fluid_test.go:26:4`。四个等价样本已内嵌为**常驻**子测试 `TestCommentIdentifierScannerCatchesKnownBadSamples`(不存在的名字必须报错并指对行/存在的名字不得报错/字段改名而注释未改必须报错/豁免名字不得报错),自检不再是一次性动作。**已上报的覆盖率疑虑**:F2 那四次真实失真里 `pending` 是**裸词**书写,本门禁按当前书写习惯抓不到;要让门禁真正生效,需要先把「注释提到标识符必须加反引号」立成书写约定(全仓 Go 注释目前只有 102 行用到反引号)。
