...
- [x] 2.3 实现待更新队列：项为 `(core.BlockPos, dueTick)`，入队时 `dueTick=now+delay`（`delay` 是 `Enqueue` 的调用参数，不是包内 tunable——归 `sim` 所有，Ruling 2），去重可用 map 但**处理前必须按 `(dueTick, ChunkKey, y, z, x)` 全序排序**。测试断言排序全序性与去重后不丢项。验证：`go test ./internal/fluid -race -count=1`
- [x] 2.4 实现 `Advance(now uint64, w FluidWorld, budget int, delay uint64) []core.BlockPos`（比 illustrative 签名多一个 `delay` 参数，同样是 Ruling 2 的要求：变化格重新入队需要 delay，且不能读包内 tunable）：本 tick 读取的存活判定只看 tick 起始状态，写入一次性提交；超预算项按原序保留；同 tick 冲突写入取流体等级最小（最强）者。验证：`go test ./internal/fluid -race -count=1`
