...
**重扫入队另有一份预算**：`FluidRescanCellsPerTick`（默认 65536）。`FluidUpdatesPerTick` 只截断
「处理队列里已有的项」，截不住「把格放进队列」这一步；而区块进入推进范围时的一次性边界重扫，
工作量正比于新进入范围的方块数（任务组 4 修复轮实测：八名玩家最坏兴趣范围一次性进入时，单 tick
花 204 ms 做入队，20 TPS 的 50 ms 预算被直接击穿）。因此重扫待办跨 tick 保留、按格数额度分摊，
计量单位是「检查过的格数」而非「入过队的格数」——跳过可证不动点后，常态是扫了几千格却一格都
没入队。分摊的安全性见 D5：不动点性质只要求重扫**最终**发生在该区块处于范围内的某个 tick。
...
**执行验证结论（任务组 3 关口）**：性质成立，且在当前规则集下**可证**而非仅「测下来没反例」——
支撑关系是良基的（水平支撑要求邻居等级严格更小，垂直支撑沿 y 严格上升且世界有界），故不存在
自持环；任何不动点必然满足「每格下方不可替换」「每个水平邻居在 level+1 下不可替换」「非源格有
支撑」，三条联立给出 `level(c) = 1 + min(水平邻居 level)`（被上方喂水时为 1），对 level 归纳即得
**不动点唯一**。

**重扫集合的承重部分是「全部流体格」，不是「空气邻居」**：`evalCell` 对非流体格恒产出空写入
（实测确认），因此能产生写入的格恰好就是全部流体格。上文「流体格及其空气邻居」中的后半句在
当前规则集下是纯冗余（保留无害，作防御性冗余）。**落地时真正必须保证的是每一个流体格都被入队，
包括跨区块边界另一侧的流体格**；漏掉邻块流体格会让不动点性质失效，漏掉空气邻居则无害。

**可进一步收缩到「能产生写入的流体格」**：水源在下方与四个水平邻格都不可被等级 1 替换时，
`evalCell` 必然返回空写入集合（源不做存活判定，垂直与水平传播的条件都落空），因此这类内部水源
是可证的不动点，重扫跳过它们不改变任何结果。海洋的重扫量由此从 O(体积) 降到 O(表面)——这不是
对完整性的放宽，而是对「能产生写入的格」这个集合的精确刻画。判据只覆盖水源：流动格的存活依赖
「上方或某个更强的水平邻格是流体」这类正向条件，读数偏保守时会把「会消失」误判成「不动」。
...
**推论一 · 互异性校验必须为 `(air, water)` 开豁免**：材料表的 FFI 层原本要求全部项两两互异
（重复 ID 会破坏分层/矿石/树逻辑中的等值比较）。但门控关闭时 `water` **就是** `air` 的编号，
两条约束直接互斥。因此校验豁免且**仅豁免** `(air, water)` 这一对：其余各项之间、以及 `water`
与其余各项之间的互异性一律照常校验。这是门控形状的直接推论，属跨语言契约的一部分，
不是实现细节——Rust 侧以 header 字节偏移写断言钉死，重排材料表顺序会让测试变红。

**推论二 · 注水只作用于整块生成路径与 `BaseBlockAt`，`TerrainBlockAt` 保持纯地形语义**：
若在 `terrain_block_at` 内注水，`base_block_at` 的「地形非空气即早返回」会吞掉后续的橡树分支，
海平面以下的树将整体消失（低洼地形的树根确实在 Y=64 以下）。因此注水叠加在地形层之后，
`TerrainBlockAt` 对海面下的空气格仍返回 air。两个入口本就分层——`TerrainBlockAt` 的语义是
「地形」而水不是地形，`BaseBlockAt` 才是「不应用会话修改时的完整方块」。该不对称由跨实现
point 差分在开启态下锁定，不会静默漂移。
