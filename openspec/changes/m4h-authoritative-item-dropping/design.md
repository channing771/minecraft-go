...
同 tick 的命令仍按序号顺序读取和修改权威选中栏位；不新增 per-command map、锁、goroutine 或事务对象。否决在客户端携带 Slot：服务端选中栏位才是权威值，客户端 slot 会制造额外陈旧状态与校验分支。否决在玩家视线前生成：掉落物当前只有方块索引，伪造投掷方向会把墙体、跨区块和碰撞问题偷偷带入本批。
