...
- [ ] 2.1 在 `cmd/mcgo` 先补充失败测试，证明消息排空上限不会扩大预热和计时帧的网格工作上限。
- [ ] 2.2 拆分帧函数的两个上限：载入阶段使用 `4096/4096`，预热和计时使用 `4096/64`，并把生产报告标记为 scenario v7。
- [ ] 2.3 运行 `zsh -ic 'gvm use go1.26.0 >/dev/null && go test ./cmd/mcgo -race -count=1'` 及相关无窗口 benchmark 检查，提交本组。
