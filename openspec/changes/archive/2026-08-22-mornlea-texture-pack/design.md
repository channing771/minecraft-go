...
客户端 `build` 还会把上述三个 notice 文件逐字节复制到 `bin/third-party/pixel-perfection/`；Linux 专服发布单元不复制它们。当前 `Makefile` 的 `CARGO := cargo` 绕过了仓库已经钉死的 Rust 1.97.1 工具链，并在 hook/CI 的精简 `PATH` 下直接以 127 失败。构建入口改为可覆盖的 `CARGO ?= rustup run 1.97.1 cargo`：默认由 rustup 选择固定版本，已有显式 `CARGO` 的环境继续拥有最高优先级，不增加安装脚本或备用下载路径。
...
- [精简 CI `PATH` 找不到 cargo] → `Makefile` 默认经 rustup 调用固定 1.97.1，并允许构建环境显式覆盖 `CARGO`。
