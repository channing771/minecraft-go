...
- [x] 5.1 在 `cmd/mcgo` 与 `cmd/perfcheck` 先写失败测试：producer 标记 v12、v11/v12 默认拒绝、仅 `10:12` 可显式迁移（v11 从未成为基线），`11:12` 与 `10:11` 均被拒绝、历史 v6-v11 可读取、v12 同场景与跨 transport 继续执行原门禁。
- [x] 5.2 修改 producer 与比较器为 scenario v12，不改变分辨率、样本数、阶段时长、绝对阈值或 `20%` 相对阈值。
- [x] 5.3 更新 `README.md` 与 `docs/notes/perf-baseline.md`，说明批量分摊计时、量化指标不做相对判定、冷却窗口、Go 堆软上限、scenario v12 与 `10:12` 迁移。
...
- [x] 7.1 在冻结候选上记录精确 HEAD、M2/M5 哈希、硬件/系统/Go、供电与负载，确认两个全新输出路径不存在且无遗留进程；向用户报告并取得 Memory/TCP 各一次、失败即停且不得重跑的明确授权。
- [x] 7.2 仅通过现有无窗口 benchmark 生成一次 M5 Memory v12 报告；用 v10 M5 基线和显式 `10:12` 执行完整性与绝对门禁，失败立即停止。
- [x] 7.3 Memory 通过后生成一次同 HEAD 的 M5 TCP v12 报告，并执行 Memory→TCP 同场景比较；失败立即停止。
- [x] 7.4 两步都通过后，把 Memory 报告精确字节写入 `docs/notes/perf-baseline-m5.json`，更新性能记录的 HEAD、命令、哈希、环境和被替代场景身份；验证 M2 文件哈希未变后提交 `chore: 建立 M5 scenario v12 基线`。
