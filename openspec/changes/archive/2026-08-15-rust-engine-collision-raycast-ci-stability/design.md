...
Rust 在清零 raycast result metadata 前 MUST 对每个非空 input/cursor/output 按固定的 40/64/1280-byte footprint 完成 checked range，只对范围有效的 buffer 检查 metadata overlap；任一 overlap MUST 不写返回。若无 overlap，MUST 先清零 metadata，再以 status 2 拒绝任一固定范围无效的 buffer；不得使用 caller-supplied length。
