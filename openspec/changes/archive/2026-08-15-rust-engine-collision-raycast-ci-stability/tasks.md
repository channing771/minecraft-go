...
- [x] 10.1 在用户明确授权发布 draft PR 后，确认 exact HEAD 的 macOS `test` 与 Ubuntu `linux-server` 均成功；日志必须证明 `libmornlea_engine.so`、`$ORIGIN`、脱离 target 参数解析和两条历史 flaky tests 均通过。首次 exact HEAD `1904300f` 的 PR run `31875600897` 因 mesh 测试间接编译 Darwin-only GPU API 而成为 source RED；修正 GPU 文件 build constraint 并重新冻结 8.1–9.2 后，exact HEAD `9f42b777` 的 run `31878887985` 已通过 macOS 与 Linux bundle 构建，但 Ubuntu identity scanner 又以 `darwin-cgo + CGO=0` 伪造 WebGPU 类型数据而成为第二条 source RED。只修正 `internal/archcheck` 测试 harness 并做比例门禁后，exact HEAD `620a4bee` 的 run `31882391843` 已通过 Ubuntu archcheck，却因 runner 未安装 `rg` 以 exit 127 成为第三条 source RED；只允许把 Linux workflow 换为系统自带 `grep` 并做同级比例门禁。以上两个 test/CI-only 修复均不得使已复核的 Attempt 6 producer/visual/perf 证据失效。最终 exact HEAD `2ae46973` 的 PR run `31882727669` 两个 job 均成功，Ubuntu 日志实际证明相邻 `.so`、`$ORIGIN`、四 symbol、脱离 target 加载与参数解析，macOS full race 未跳过两条历史 fixture。此项完成后才可同步主 specs 并用官方 archive workflow 归档；不得自行 push、merge 或 archive。
