...
### 8. scenario v10 显式迁移且正式基线只在静稳宿主上执行一次
...
M2 基线及路径保持不变。M5 在实现、全仓门禁和计划提交后冻结候选 HEAD。最后一个完整门禁进程退出后自然冷却至少 5 分钟，再间隔至少 30 秒采集两次宿主状态；两次的 1 分钟和 5 分钟 load average 都必须小于 `6.0`，同时必须使用 AC 供电、关闭低电量模式、电池不少于 `50%`，且没有遗留 `mcgo` 或 `perfcheck` 进程。预检通过后才报告精确 HEAD、全新报告/日志路径和环境证据并请求一次性授权。

预检是正式链外的只读流程，不写入 benchmark producer，也不新增一次性脚本或命令包。这样通用 benchmark 仍可用于诊断，正式基线的环境约束则留在可审计的执行计划内。预检失败时不主动终止用户进程、清理系统缓存或改写供电状态，只停止并保留证据；再次进行只读预检不算重跑正式 benchmark。用户授权后、Memory 启动前再复核相同条件，任一条件变化都停止并重新预检、重新授权。

通过复核后用两个全新报告路径及各自日志路径先执行一次 Memory，再在通过后执行一次 TCP；任一步失败停止且不重跑。全部通过后才把 Memory 精确字节写入 M5 基线并记录哈希和被替代的 v9 身份。HEAD `01d28d9f5b4eeedee4200bb62f35a42ca7c1d83c` 的旧正式链已经在唯一一次 Memory 运行中因 flying p99 `31.152ms` 失败；该日志与后续非正式诊断只作为失败证据，不得复制、改名或提升为新基线。流程修订形成新提交后，必须重新完成候选门禁并使用新 HEAD、全新路径和新授权开始独立正式链。

否决把宿主预检加入 producer：这会让通用诊断 benchmark 依赖 macOS 电源状态，并把一次性发布纪律扩散到产品命令。也否决新增 shell/Go 预检工具：当前只有一条受控正式链，计划中的固定只读命令已经可审计，新增解析器没有复用价值。
...
- [宿主后台负载污染正式性能证据] → 完整门禁后自然冷却，授权前双采样并在 Memory 启动前复核；不满足条件时不启动 producer、不消耗正式运行机会。
...
4. 冻结候选提交，完成冷却、双采样和启动前复核后取得一次性授权，再按 Memory 后 TCP 顺序建立 M5 v10 基线；两步都通过后才提升基线。
