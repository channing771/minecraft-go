...
- [x] 8.1 `make rust` 与 `make rust-check` 后运行 `go test ./... -race -count=1`。**已知既有红灯不得修改、不得改阈值**:`TestChatCommandAddressesExactConfiguredCompanionAtTickBoundary`(单独 `-run` 时约 9/10 失败、全包跑时通过,测试隔离缺陷)、`TestDroppedItemSurvivesShutdownAndRestart` 与 `TestAuthoritativeMiningMemoryLifecycle`(90 秒超时,macOS 时序抖动)。出现这三条之外的失败必须上报。 **前置**：`farmingLoginBudget` 600 → 3000（Ruling 44，实测卡点是异步区块生成，空闲机器约 202 tick、满载 300+；该预算唯一职责是把挂起变成可读失败）。 **本次实际观察**：全量竞态跑了三次。第一次 EXIT=1——`cmd/mornlea` 包级 10 分钟默认超时（600.651s，超时时正跑 `TestScenarioV12GPUCompletionAmortizesOverFixedBatch`）与 dialogue 族的 `TestCompanionDialogueStaleOutcomeDiscarded`；第二次 EXIT=1——前两者都转绿，改由 `TestMornleaCurrentIdentity` 因 `.codex/hooks.json` 被本代理之外的并发进程从工作区删除而报错；恢复该已跟踪文件后第三次 **EXIT=0**，23 个包全绿、零 FAIL。`cmd/mornlea` 单独跑三次分别是 61.9s / 62.0s / 62.3s，只有第一轮达到 600s（约 10 倍），那一轮中途用户的 Codex 桌面端启动，GPU 用例被并行包饿死——判为环境竞争而非本分支缺陷，但它不在本任务列出的已知红灯清单里，已上报。已知红灯一条未改、阈值一个未动、也没有加 `-timeout` 去回避那次包级超时。
...
- [x] 9.1 `internal/archcheck` 新增检查:**注释中提及的标识符必须存在**。F2 里此类失真出现**四次**,其中一次就在删掉该字段的同一个 commit 里,另一次在刚做过该文件专项检查的同一轮——已证实人工检查挡不住。 **结论**:新增 `TestCommentBacktickIdentifiersExist`,扫全仓 642 个 Go 文件(含 `_test.go`)的注释,取**反引号包裹**且形如 Go 标识符或点路径的 token,点路径只查末段,判定其是否出现在全仓声明名集合(包名/函数与方法名/类型名/常量与变量名/结构体字段名/接口方法名)。裸词不解析:实测全仓 5,442 处 CamelCase 裸词里 516 处(169 个不同 token)解析不到,全部是误报(常量名口语化简写、里程碑代号 M5C、Rust/WGSL 常量、标准库名与 KiB 一类单位),豁免成本远高于门禁价值。豁免为显式清单 + 理由:4 条非 Go 名字(`MGW1` wire magic、`mornlea_engine`/`mornlea_client` crate 名、`is_plant` Rust 函数名)、4 类文件扩展名末段(md/go/rs/wgsl)、1 条「全大写 + 下划线」形状豁免;后者另有 `TestShoutingExemptionHidesNoRealGoName` 守住其前提(Go 侧不产生该形状名字),一旦有人声明 FOO_BAR 该豁免自己失效。**全仓当前 0 条真失真**:35 处反引号提及中 28 处落入豁免、7 处解析成功。已知盲区写进文件顶部 GoDoc:计数漂移(9.0 修的三处就是这类,句子里没有标识符)、裸词失真、语义失真、Rust/WGSL 注释(后续扩展点)。
- [x] 9.2 **先用已知坏样本自检检测器**再信任其「0 条」结论——一个恒真的扫描器与不扫描等价,而它看起来像扫过了。验证:`go test ./internal/archcheck -count=1` **结论**:扫描器内核 `scanCommentBacktickIdentifiers` 吃 map[文件名][]byte、不碰磁盘,自检因此与全仓门禁**走同一条代码路径**。三个真实仓库样本逐个验过并已移除:(a) 在 `internal/sim/engine.go` 真实注释里写一个不存在的名字 → EXIT=1,报告精确指到 `engine.go:103:27`;(b) 改成两个存在的名字(`advanceWorldTime`、`core.ChunkKey`) → EXIT=0;(c) 复刻 F2 坏样本的**形状**,把 `internal/worldgen/oracle_test.go` 的 `oracleSeaLevel` 改名为 oracleSeaLevelY 而 `fluid_test.go:26` 的注释不改 → EXIT=1,指到 `fluid_test.go:26:4`;该样本能红的前提是旧名在全仓**唯一**。四个等价样本已内嵌为**常驻**子测试 `TestCommentIdentifierScannerCatchesKnownBadSamples`(不存在的名字必须报错并指对行/存在的名字不得报错/字段改名而注释未改必须报错/豁免名字不得报错),自检不再是一次性动作。**Ruling 54(同名掩盖)**:末段是在全仓 9000 多个名字的扁平集合里查的,短小通用名会被任何无关同名声明掩盖——`Queue.pending` 因 `VisibilityScratch` 也有 pending 字段而永远解析得到,故 F2 那次即便加了反引号也抓不到。裁决为**不改判定规则**(真收紧需要跨包类型解析,成本远超门禁价值),改为写进 GoDoc 盲区第五条,并新增 `TestShadowedLastSegmentIsAKnownBlindSpot` 把该行为钉成已知且有意(断言「必须绿」,守的是文档与实现的一致性:谁把判定收紧了它就红,提醒同步更新盲区)。**已上报的覆盖率疑虑**:F2 那四次真实失真里 `pending` 是**裸词**书写,本门禁按当前书写习惯抓不到;要让门禁真正生效,需要先把「注释提到标识符必须加反引号」立成书写约定(全仓 Go 注释目前只有 102 行用到反引号)。
