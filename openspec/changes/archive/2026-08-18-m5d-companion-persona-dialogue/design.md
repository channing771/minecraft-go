...
**裁决（终态折入与 FIFO 负载下的终态丢弃）**：进展节点选择公式刻意覆盖末段（`floor(i*n/6)` 的 i=6 即末步），末个选中步骤的完成迁移产出 `TaskCompleted`，其「完成表达」由同一迁移点派发的终止台词承载——因此十二步任务实际发起 1+5+1=7 次而非字面 8 次，`≤8` 是硬上界。终态台词结果经世代判定应用：FIFO 中还有后续任务（或结果在途期间队列被填满）时，同 tick 的 BeginHead 已提升世代，迟到的终态结果按过时丢弃——连续任务串下只有队尾任务的终态摘要落地。这是刻意的：旧任务的终态摘要乱序覆盖新任务的摘要比少一次摘要更糟（与「过时 MUST 被丢弃」同一语义）。
...
- 共享模型槽：现有 Planner 4 槽并发闸改造为 `modelGateway`（命名以实现为准）：`acquire(ctx)`（阻塞等待，Planner 用）与 `tryAcquire()`（非阻塞，Dialogue 用）。关服 cancel 同时唤醒两者。
  实现校准：落地方案未新增 gateway——Planner 沿用 M5B 既有的 tick 边界 try-acquire + 下一 tick 重试（即 tick 安全的「等待槽位」），Dialogue 复用同一信号量走 try-acquire 失败即跳过；语义与上述 acquire/tryAcquire 划分等价，且对 Planner 路径零改动。
