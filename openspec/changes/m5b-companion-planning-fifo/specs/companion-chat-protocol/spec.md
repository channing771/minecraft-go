...
#### Scenario: 非法目标、未知目标与队列满可区分

- **GIVEN** 目标超过名称边界的命令、目标合法但未配置的命令与目标合法但 FIFO 已满的命令
- **WHEN** 服务端在 tick 边界处理三者
- **THEN** 三条 MUST 分别得到 InvalidFormat、UnknownCompanion 与 QueueFull，且都 MUST NOT 执行指令文本或调用模型
