...
| 7 | `/root/texture_task7_visual` | 实现 PASS/CLEAN（round 2）；整体 BLOCKED | 实现 PASS/CLEAN（round 2）；整体 BLOCKED | 2 | 关闭 1×P3：把 `nearBandGuard` 的旧 golden/new frame 注释与命名改为同 registry 的 LOD-off/LOD-on current control 语义，不改几何、行带、RGB 比较或阈值。三 application preflight 已通过独立评审；沙箱无 GPU adapter，14 张 golden 零变化，真实 visual-update/check 与逐图验收仍未完成，7.1–7.7 保持未勾。 |
| 8 | `/root/texture_task8_baseline` | 待独立规格评审 | 待独立质量评审 | 1 | implementer 已保存 identity RED，仅删除 `.gitignore` 的冗余旧身份项 `/mcgo`；full archcheck 进一步暴露 Task 7 注释把局部参数写成反引号标识符，已只去掉 `lodOff`/`lodOn` 的反引号并恢复完整 archcheck。长期能力、归属链接与固定版本已对账；Rust、可运行 Go race、vet、build、OpenSpec 及非 GPU capture control 门禁已执行。当前 sandbox 禁止 TCP/httptest bind，且无 GPU adapter；final visual-check 精确失败为 `启动失败: client: 本机无可用 GPU 适配器`，golden 零改动。7.1–7.7 与 8.1–8.7 全部保持未勾，等待双评审、有 GPU 环境视觉验收与 controller ruling；本行不作 PASS/CLEAN 结论。 |
