...
| 7 | `/root/texture_task7_visual` | pending | pending | 0 | 三 application update-only control 与 lifecycle/fail-before-write 测试已完成；focused `-race` PASS，漏关 LOD-on mutation 被测试抓住。视觉裁决 BLOCKED（环境）：`visual-check` 与 `visual-update` 均在首个 offscreen application 构造时报 `client: 本机无可用 GPU 适配器`，三项仓库真实 renderer/LOD offscreen 测试亦因此 SKIP；现有六类 baseline 只作参考，不接受为新默认材质结果，golden 变更 0。须在可用 Metal adapter 上完成 14 场景 update/check/逐图验收后再勾 7.1–7.7。 |
