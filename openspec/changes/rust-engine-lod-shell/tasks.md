...
- [ ] 2.1 `ffi.rs` 出口 `mornlea_lod_shell`(入口校验、两段式 overflow、
      ...
      -- -D warnings`。
- [ ] 2.2 `internal/nativeabi` 绑定与输入输出编码测试;probe 高度差分与
      壳覆盖结构测试(oracle 保留方案);
      验证:`go test ./internal/nativeabi -race -count=1`(差分测试随
      `internal/lod` 落地后一并运行)。
- [ ] 2.3 新建 `internal/lod` 包并登记 `internal/archcheck` 依赖;
...
- [ ] 4.1 `mornlea_client`:`render_upload_lod_tile`/`render_drop_lod_tile`
...
- [ ] 4.2 `internal/client` 上传绑定与测试;
      验证:`go test ./internal/client -race -count=1`。

## 5. Go 编排与接线

- [ ] 5.1 `internal/lod.Scheduler`:环形入队、pending 覆盖、距离升序
