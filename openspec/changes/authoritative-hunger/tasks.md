...
- [ ] 6.1 `internal/server` 端到端(Memory):从农业闭环接力——缺失玩家登录 → 跳跃 N 次直到饥饿 < 18 且回血停止 → 徒手采石/翻地/种/收小麦 ×3(或夹具给 3 小麦,写明)→ 合成面包 → 长按 32 tick → 饥饿回升 → 回血恢复。**每步精确数列**。验证:`go test ./internal/server -race -count=1`
- [ ] 6.2 变异验证:去掉回血门控 → 端到端「跳到饿后回血停止」那一步红;面包饥饿值改 0 → 「进食后回升」红。
