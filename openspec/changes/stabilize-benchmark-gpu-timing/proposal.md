...
- 让 `internal/gfx` 暴露 GPU 时间戳查询能力，`remote_gpu_complete` 改为记录 GPU 侧真实执行时间，摆脱 `Poll` 往返节拍；设备不支持该能力时以可观察的方式明确失败，不静默回退到旧的墙钟测量。
...
非目标：不改变 still/flying 时长与相机脚本、不改变 `2560x1440` 分辨率、不改变 2048 个样本数、不放宽任何既有绝对门禁、不引入自动硬件探测或跨硬件归一化、不修改 M2 基线。
...
- 图形封装：`internal/gfx`（新增时间戳查询抽象，仍是唯一可直接 import WebGPU 绑定的包）。
