...
新增稳定编号集合上线时，线上协议语义版本 SHALL 升至 v14，玩家存档 schema SHALL 升至 v6，区块存档 schema SHALL 升至 v7；三者字节布局 MUST 保持不变。玩家 v5→v6 与区块 v6→v7 MUST 执行 identity migration，保留既有背包、耐久、位置、生命值、palette、掉落物、熔炉、箱子和 revision，且不得向已有玩家注入材料包。
...
玻璃和树叶 SHALL 是使用标准完整方块碰撞的可绘制 cutout 方块。它们 MUST NOT 完全遮挡 AO 或客户端派生天空光；相同玻璃之间和相同树叶之间 MUST 剔除内部面。不透明方块与 cutout 方块相邻时 MUST 保留可透过孔洞观察到的不透明方块面；两种不同 cutout 方块相接时 MUST 不生成重叠的共面内部表面。
