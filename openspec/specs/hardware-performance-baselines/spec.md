...
- **GIVEN** M4L 基线合并与 scenario v13 生产代码和计划已提交，且 Memory/TCP 使用两个全新临时路径
...
- **GIVEN** 一个 scenario v13 候选仍基于 M4J，而目标分支已经归档 M4L
- **WHEN** 项目准备建立 M4I 正式基线
- **THEN** 该候选及其诊断输出 MUST 只保留为不可提升证据；项目 MUST 先合并 M4L、重新完成门禁与静稳预检，并为新的精确 HEAD 和两个全新路径取得明确授权
