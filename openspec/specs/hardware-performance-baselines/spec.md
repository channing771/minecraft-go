...
### Requirement: 同硬件比较保留完整性校验并记录性能

对于同硬件、同 transport、同 scenario 的完整报告，比较器 MUST 继续校验字段完整性和身份；所有统计指标、最小有意义增量和相对变化 MUST 作为性能记录输出，不得作为失败条件。

#### Scenario: 统计退化只记录
- **WHEN** 同硬件、同 transport、同 scenario 的完整报告出现任意指标退化
- **THEN** 比较器 MUST 输出对应记录并返回成功
