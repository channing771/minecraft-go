...
当前代码基线是 M3B，已经包含有界版本化二进制协议、Memory/TCP 共用登录状态机、TCP 直连、无图形专用服务端、稳定玩家身份和玩家状态存档。当前只允许一个 Play 玩家，TCP 仅面向可信局域网且没有认证或加密。`docs/superpowers/` 同时保存了已实现里程碑和未来里程碑的设计/计划；文档写了某项能力不代表代码已经实现，判断现状时必须检查代码和测试。
